...
public class Admin extends Person {
    ...
    //         System.out.println("order number: "+(i+1)+"total price: "+Database.Orders.get(i).orderCart.totalprice+"payment method: "+ Database.Orders.get(i).getPaymentMethod());
    //     }
    // }
    ...
    public void bancustomer(String username){
        ...
        for(int i = 0; i < Database.Customers.size(); i++){
            if(Database.Customers.get(i).getUsername().equals(username)){
                Database.Customers.get(i).setBanned(true);
                return;
            }
        }
